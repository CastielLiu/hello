...
def main():
	...
	while True:
		...
		#print('index = %d len = %d' %(index,pointNum))
		indexOffset = 1
		
		t_roadWheelAngle = t_roadWheelAngleList[index+indexOffset] + p1*latErr + p2*yawErr
		...
		angleList.append(vehicleState.roadWheelAngle)
		
		#ax1.plot(vehicleState.x,vehicleState.y,'b.')
		#plt.pause(0.001)
		
		f_.write('%.2f\t%.2f\t%.2f\t%.2f\r\n' %(vehicleState.x,vehicleState.y,latErr,yawErr*180.0/math.pi))
		
		if(pointNum <= index+indexOffset+5):
			break
	f_.close()
	
	ax1.plot(trackingEffect_x,trackingEffect_y,'b-')
	ax2 = fig.add_subplot(312)
	ax2.plot(trackingEffect_x,latErrList,'r-',label='latErr',linewidth=0.5)
	ax2.grid()
	plt.ylim(-1,1)
	plt.xlim(0,30)
	...
	ax3.plot(trackingEffect_x,[x*180.0/math.pi for x in yawErrList],'r-',label='yawErr')
	ax3.plot(trackingEffect_x,[0]*len(trackingEffect_x),'k-')
	
	yawErrList = [math.fabs(x*180/math.pi) for x in yawErrList]
	YawMainErr = reduce(lambda x,y:x+y ,yawErrList)*1.0/len(yawErrList)
	ax3.text(5,0,'YawMainErr:%.2fdeg'%(YawMainErr))
	plt.ylim(-20,20)
	plt.xlim(0,30)
	'''
	ax4 = fig.add_subplot(414)
	ax4.plot(trackingEffect_x,[x*180.0/math.pi for x in angleList],'r-')
	ax4.plot(trackingEffect_x,[0]*len(trackingEffect_x),'k-')
	'''
	plt.savefig(test_name+'.pdf')
	with open('debug.txt','a') as f:
		f.write('%.2f\t%.2f\t%.2f\t%.2f\r\n' %(p1,p2,vehicleState.speed,LatMainErr))
